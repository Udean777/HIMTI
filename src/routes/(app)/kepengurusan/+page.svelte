<script lang="ts">
	import { divisionImages, kepengurusan } from '$lib/dummy';
	import { quintOut } from 'svelte/easing';
	import { fly, fade } from 'svelte/transition';

	const divisionDetails: Record<string, string> = {
		BPH: 'Badan Pengurus Harian yang menjadi inti dari organisasi, bertanggung jawab atas arah strategis dan operasional HIMTI.',
		HUMAS:
	...
	const divisionHead = $derived(
		kepengurusan.find(
			(m) =>
				...
				(m.role.includes('Kepala Divisi') || m.role.includes('Ketua Umum'))
		)
	);

	const divisionMembers = $derived(
		kepengurusan.filter((m) => m.division === activeDivision && m.name !== divisionHead?.name)
	);
		...
		{#key activeDivision}
			<div in:fade={{ duration: 400, delay: 150 }} out:fade={{ duration: 150 }}>
					...
					<div class="relative flex h-full min-h-[400px] flex-col justify-end p-6 md:p-8">
						<div class="text-white">
							<h2 class="text-4xl font-bold">
								{activeDivision !== 'BPH' ? `Divisi ${activeDivision}` : activeDivision}
							</h2>
							<p class="mt-2 max-w-2xl text-slate-300">{divisionDescription}</p>
						</div>
					</div>
				</div>
					...
					>
						...
						>
							...
							<div class="p-4">
								...
								<p class="text-muted-foreground text-xs">Periode {divisionHead.period}</p>
							</div>
						</div>
					</div>
				{/if}
				...
				{#if divisionMembers.length > 0}
					<h3 class="text-foreground mb-8 text-center text-3xl font-semibold">Anggota</h3>
					<div class="flex flex-wrap justify-center gap-6">
						{#each divisionMembers as person, i (person.name)}
							<div
							...
							>
								...
								<div class="p-4">
									...
									<p class="text-muted-foreground text-xs">Periode {person.period}</p>
								</div>
							</div>
						{/each}
					</div>
